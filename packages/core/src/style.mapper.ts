  ...
  protected initializeDefaultMappings(): void {
    this.mappings = {
      ...
      border: (v: string, el) => {
        ...
        // Parse border shorthand for Table and Block elements (Paragraphs)
        // We previously only allowed 'table', but for custom inheritance we need to support others.
        const parts = raw.split(/\s+/);
        let width = 0;
        ...
        parts.forEach((part) => {
          // Check for width (ends in px, pt, cm, etc or is a number)
          if (
            /^\d+(\.\d+)?(px|pt|cm|mm|in|pc|em|ex|ch|rem|vw|vh|vmin|vmax|%)?$/.test(
              part
            )
          ) {
            const px = parseFloat(part); // simplistic, assumes px if no unit or parses prefix
            if (!isNaN(px)) width = px * 8; // approx conversion to eighths of point
          }
          // Check for style
          else if (
            [
            ...
            style = mapBorderStyle(part) as any;
          }
          // Assume parsing color is complex, so if it's not width or style, try color
          else {
            const parsedColor = colorConversion(part);
            if (parsedColor !== '000000' || part.toLowerCase() === 'black') {
              color = parsedColor;
            }
          }
        });

        if (parts.length > 0 && width > 0) {
          const borderSpec = {
            style,
            size: width,
            color,
          ...
          // If it's a paragraph or other block, docx expects 'border' (singular) with top/bottom/left/right
          // If it's a table, docx expects 'borders' (plural)
          // Actually, for docx Paragraph properties, it is 'border: { top: ..., bottom: ... }'
          // For Table properties, it is 'borders: { top: ..., bottom: ... }'
          const borderObj = {
            top: borderSpec,
            bottom: borderSpec,
            left: borderSpec,
          ...
          if (el.type === 'table') {
            return { borders: borderObj };
          } else {
            // For paragraphs, the property is 'border'
            return { border: borderObj };
          }
        }
        return {};
      },
